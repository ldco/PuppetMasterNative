...
- `PMN-074` admin roles provider-backed list path has started:
  ...
  - admin logs export mutation slice has started (`backend.genericRest.admin.endpoints.exportLogs`, config-gated) with provider/service/hook/screen support (JSON/CSV actions, result handling for direct URL vs async jobId)
  - admin log export job-status slice has started (`backend.genericRest.admin.endpoints.getLogExportJob`, config-gated) with provider/service/hook/screen support for queued/running/ready/error polling
...
- Full provider-backed admin module APIs are not implemented yet (users list/detail, roles list, logs list + clear logs + export + audit actions [acknowledge/resolve/retry], admin settings snapshot, role update, user status update, user lock/unlock, user sessions/session-revoke workflows, and health snapshot are started; deeper admin workflows remain pending, though list-level ops UX is now improved)
...
3. `PMN-074` Admin: continue from users list/detail + roles list + logs list + clear logs + export (JSON/CSV UI wired) + export job status checks + per-log audit actions (`acknowledge/resolve/retry`) + admin settings snapshot + role update + user status + user lock + user sessions/session-revoke workflows + health (provider-backed generic-rest paths started, plus inline users-list ops controls and destructive-action confirmations) to broader admin workflows/endpoints (policy actions, richer export filters/download governance, force logout reasons/audit metadata, session expiry/device metadata, etc.) with provider tests/contracts first.
