...
- Full provider-backed admin module APIs are not implemented yet (users list/detail, roles list, and role update are started; admin settings/other endpoints and deeper workflows remain pending)
...
3. `PMN-074` Admin: continue from users list/detail + roles list + role update (provider-backed generic-rest paths started) to next endpoints/actions (`settings`, user status/locks, audit/logs) with provider tests/contracts first.
